...
pub async fn compute_chunk_group_info(graph: &ModuleGraph) -> Result<Vc<ChunkGroupInfo>> {
    ...
    async move {
        ...
        let mut chunk_groups_to_id: FxHashMap<ChunkGroup, ChunkGroupId> = Default::default();
        let mut chunk_groups_from_id: FxHashMap<ChunkGroupId, ChunkGroup> = Default::default();
        ...
        let mut module_depth: FxHashMap<ResolvedVc<Box<dyn Module>>, usize> = Default::default();
